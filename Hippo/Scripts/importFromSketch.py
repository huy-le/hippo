...
if not len(sys.argv) == 4:
    print "Proper usage: python slice.py [.sketch file] [.xcassets file] [folder name]"
    exit(1)
...
ASSETS_FOLDER_FRIENDLY = sys.argv[3]
ASSETS_FOLDER = "_%s" % ASSETS_FOLDER_FRIENDLY

EXPORT_DIR = os.path.join(os.path.dirname(INPUT_FILE), 'tmp_exported_slices')
OUTPUT_DIR = os.path.join(ASSETS_FILE, ASSETS_FOLDER)
...
if sketchtool_executable is None:
    print 'Sketchtool not found; skipping'
    exit(0)
...
for asset_group in asset_groups.values():
    ...
    for asset in asset_group.assets:
        assetNames[asset.filename.split("@")[0].split(".")[0]] = True
...
extension = "// This file is auto-generated by the importFromSketch.py script.\n// Any changes made here will be blown away next time the Sketch file is imported.\n\nextension Style {\n\n    public enum %s: String, UIImageConvertable {" % ASSETS_FOLDER_FRIENDLY
for key in sorted(assetNames.keys()):
    caseName = key[0].lower() + key[1:]
    extension += "\n        case %s = \"%s\"" % (caseName, key)
extension += "\n    }\n}"

f = open("%s/../%s.swift" % (ASSETS_FILE, ASSETS_FOLDER_FRIENDLY), 'w')
f.write(extension)
